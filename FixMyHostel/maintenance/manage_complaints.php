...
if ($_SESSION['role'] != "maintenance_associate" && $_SESSION['role'] != "maintenance_supervisor") {
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'unread_counts') {
    ...
    $unreadResult = $conn->query("
        ...
        WHERE c.department='Maintenance'
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center flex-nowrap">
                                                    ...
                                                    <?php if ($row['department'] == 'Maintenance') { ?>
                                                        ...
                                                        <?php if (($currentRole === 'maintenance_associate' || $currentRole === 'maintenance_supervisor') && $row['status'] != 'resolved') { ?>

                                                            <!-- Status Form -->
                                                            <form method="POST" class="d-flex align-items-center">
                                                                <input type="hidden" name="complaint_id" value="<?php echo $row['complaint_id']; ?>">
                                                                <select name="status" class="form-select form-select-sm me-2 pr-2" style="width:auto;">
                                                                    <?php 
                                                                        if ($currentRole === 'maintenance_associate') {
                                                                            $statuses = [
                                                                                'pending' => 'Pending',
